...
def load_models():
    """
    Loads BERT (Category) and RF (Urgency) models.
    Returns a dict of models or None if files are missing.
    """
    models = {}
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    try:
        # --- Load Model 1: DistilBERT (Categorization) ---
        # Assuming folder './final_model3' exists
        bert_tokenizer = AutoTokenizer.from_pretrained("./final_model3")
        bert_model = AutoModelForSequenceClassification.from_pretrained("./final_model3")
        ...
        # --- Load Model 2: Poly SVC (Urgency) ---
        # Assuming folder './trained_priority_models' exists
        rf_model = joblib.load('trained_priority_models/poly_svc_model.joblib')
